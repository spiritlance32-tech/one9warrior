...
def generate_receipt(
    ...
    # Use payment date (start_date) for folder names
    if hasattr(start_date, "strftime"):
        payment_date = start_date
    else:
        payment_date = datetime.strptime(str(start_date), "%d %b %Y")

    year = payment_date.strftime("%Y")
    month = payment_date.strftime("%B")

    # Timestamp so receipts never overwrite each other
    timestamp = datetime.now().strftime("%d%m%Y_%H%M%S")

    file_name = (
        f"{year}/"
        f"{month}/"
        f"{safe_name}_-_{safe_plan}_-_{month}_{year}_-_{timestamp}.pdf"
    )
