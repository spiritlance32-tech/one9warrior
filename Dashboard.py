...
from database import get_all_members, add_new_member, renew_member, delete_member, update_member_status
from receipt import generate_receipt
from helpers import make_receipt
    ...
    def show_cards(target_df, tab_id):
        ...
        for _, row in target_df.iterrows():
            ...
            with st.expander(f"{status} {row['full_name']} | Exp: {row['expiry_date'].strftime('%d %b %Y')}"):
                ...
                if st.button(
                    "💾 Update Status",
                    key=f"status_btn_{kid}",
                    use_container_width=True,
                ):
                    update_member_status(row["id"], status)
